...
export const listCategories = () => async (dispatch) => {
  try {
    const data = await axios.get('https://blog-backend-86a4.onrender.com/blogCategories');
...
export const allBlogs = () => async (dispatch) => {
  dispatch(setLoading(true));
  try {
    const response = await axios.get('https://blog-backend-86a4.onrender.com/api/read/blog');
    console.log(response);
    dispatch(setBlogs(response.data.blogs));
